package cn.US.controller;

import cn.US.bean.Pay;
import cn.US.bean.dto.DataTablesResult;
import cn.US.bean.dto.PageVo;
import cn.US.bean.dto.Result;
import cn.US.common.utils.*;
import cn.US.dao.PayDao;
import cn.US.service.PayService;
...
public class PayController {
    ...
    private static final String CLOSE_KEY="USpay_CLOSE_KEY";
    ...
    public Result<Object> addPay(@ModelAttribute Pay pay, HttpServletRequest request){
        ...
        try {
            if(pay.getCustom()!=null&&pay.getCustom()&&!"UnionPay".equals(pay.getPayType())&&!"Diandan".equals(pay.getPayType())){
                ...
                // 从redis中取出num
                String key = "USpay_NUM_"+pay.getPayType();
        ...
        emailUtils.sendTemplateMail(EMAIL_SENDER,EMAIL_RECEIVER,"【USpay个人收款支付系统】待审核处理","email-admin",pay);
        ...
        if(StringUtils.isNotBlank(pay.getTestEmail())&&EmailUtils.checkEmail(pay.getTestEmail())){
            ...
            emailUtils.sendTemplateMail(EMAIL_SENDER,pay.getTestEmail(),"【USpay个人收款支付系统】待审核处理","email-fake",pay2);
        }
        ...
        try {
            ...
            if(StringUtils.isNotBlank(pay.getEmail())&&EmailUtils.checkEmail(pay.getEmail())){
                if("0".equals(sendType)){
                    emailUtils.sendTemplateMail(EMAIL_SENDER,pay.getEmail(),"【USpay个人收款支付系统】支付成功通知","pay-success",pay);
                }else if("1".equals(sendType)){
                    emailUtils.sendTemplateMail(EMAIL_SENDER,pay.getEmail(),"【USpay个人收款支付系统】支付成功通知","sendwxcode",pay);
                }else if("2".equals(sendType)){
                    emailUtils.sendTemplateMail(EMAIL_SENDER,pay.getEmail(),"【USpay个人收款支付系统】支付成功通知","sendxboot",pay);
                }
            }
        }catch (Exception e){
    ...
    public String passNotShowPay(@RequestParam String id,
        ...
        try {
            ...
            //通知回调
            if(StringUtils.isNotBlank(pay.getEmail())&&EmailUtils.checkEmail(pay.getEmail())){
                emailUtils.sendTemplateMail(EMAIL_SENDER,pay.getEmail(),"【USpay个人收款支付系统】支付成功通知","pay-notshow",pay);
            }
        }catch (Exception e){
            model.addAttribute("errorMsg","处理数据出错");
            return "500";
    ...
    public String backPay(@RequestParam String id,
        ...
        try {
            ...
            Pay pay=payService.getPay(getPayId(id));
            if(StringUtils.isNotBlank(pay.getEmail())&&EmailUtils.checkEmail(pay.getEmail())){
                emailUtils.sendTemplateMail(EMAIL_SENDER,pay.getEmail(),"【USpay个人收款支付系统】支付失败通知","pay-fail",pay);
            }
        }catch (Exception e){
            model.addAttribute("errorMsg","处理数据出错");
            return "500";
